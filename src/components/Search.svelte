...
">
    <span class="mono-search-prefix" aria-hidden="true">[/]</span>
    <input aria-label="搜索文章" placeholder="SEARCH ARCHIVE" bind:value={keywordDesktop} on:focus={() => search(keywordDesktop, true)}
...
<button on:click={togglePanel} aria-label="Search Panel" id="search-switch"
        class="btn-plain lg:!hidden w-11 h-11 active:scale-90">
...
top-20 left-4 md:left-[unset] right-4 shadow-2xl rounded-2xl p-2">
    <div class="mono-section-head -mx-2 -mt-2 mb-2">
        <h2>Search index</h2>
        <span class="mono-badge">RSS</span>
    </div>
  ...
  ">
        <span class="mono-search-prefix" aria-hidden="true">[/]</span>
        <input aria-label="搜索文章" placeholder="SEARCH ARCHIVE" bind:value={keywordMobile}
    ...
    {#each result as item}
       ...
       rounded-xl text-lg px-3 py-2 hover:bg-[var(--btn-plain-bg-hover)] active:bg-[var(--btn-plain-bg-active)]">
             <div class="transition text-90 inline-flex font-bold group-hover:text-[var(--primary)]">
                 {item.meta.title}<span class="mono-result-open">[OPEN]</span>
            </div>
