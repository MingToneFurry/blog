...
const postPaths = homeDocument
	.querySelectorAll('[data-stats-scope="post"]')
	.map((node) => node.getAttribute("data-stats-path"))
	.filter(Boolean);

assert.equal(postPaths.length, 8, "live verification expects all 8 homepage post paths");
...
async function createHelperWindow() {
	...
	assert.equal(storage.getItem("umami-share-cache:legacy"), null, "legacy share cache must be purged");
	return sandbox;
}
...
async function verifyRound(round) {
	...
	async function worker() {
		while (cursor < targets.length) {
			...
			try {
				const stats = await withRetry(() => helper.fetchUmamiStats(BASE_URL, SHARE_ID, {
					timezone: TIMEZONE,
					...(postPath ? { url: postPath } : {}),
				}));
				assert.notEqual(getNumericValue(stats.pageviews), null, `${postPath || "site"} PV missing`);
				assert.notEqual(getNumericValue(stats.visitors), null, `${postPath || "site"} UV missing`);
	...
	console.log(`round ${round}: ${results.length - failures.length}/${results.length} cumulative PV+UV targets ready`);
	if (failures.length > 0) {
		console.error(`failed paths: ${failures.map((failure) => failure.path).join(", ")}`);
		throw failures[0].error;
	}
}

for (let round = 1; round <= 3; round += 1) await verifyRound(round);
console.log("ARCADE live Umami verification passed without persisting or printing the share token");
