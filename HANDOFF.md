...
- 2026-07-29 主线程在应用内浏览器以 `1440x1000` 与精确 `390x844` 复验：首页无横向溢出，全站与当前 8 篇文章统计连续三轮均为 9/9 `ready`、0 `error`、0 非法值；全站快照依次为 `335865 / 140105`、`335866 / 140105`、`335867 / 140105`（PV / UV，数值会继续增长）。
- 中文文章 `/posts/已为本站支持自动清除cloudflare缓存/` 累计统计为 `63 PV / 15 UV`，文章目录可见且点击后 hash 与滚动位置同步；`Alt+→` 可切换到较旧文章，Swup 页面切换及浏览器前进/后退均正常。
- 搜索 `cloudflare` 返回 3 条站内文章；主题、背景显隐和柔化值经重载保持，随后已恢复为跟随系统、背景显示、柔化 `4`。背景故障注入依次确认主源 `api.furry.ist`、SNI `sni-api.furry.ist`、双失败 `error` 且空 `src`、解除阻断后恢复主源。
- `49438d5` 修复后已真实复验：搜索 Dialog 接收 Escape 后关闭，焦点回到“搜索”触发按钮。页面日志无应用错误，仅有 Giscus 在尚无讨论时的预期 warning。
- 最终截图目录：`C:\Users\MingTone\.codex\visualizations\2026\07\27\019fa50e-1c97-71e2-a08c-fab02dbda466\blog-rebuild-qa`；关键文件为 `mono-final-home-desktop.png`、`mono-final-home-mobile.png`、`mono-final-article-desktop.png`。记录和截图均不包含动态分享令牌。
